...
Proof. apply limit_to_a_equiv with (f1 := λ x, x^2 + 2 * x + 4); auto_limit. Qed.
...
Proof.
  ...
  - simpl. apply limit_to_a_equiv with (f1 := λ x, 0); auto_limit.
  - replace (S k - 1)%nat with k by lia. apply limit_to_a_equiv with (f1 := λ x, x^k + y * ((x^k - y^k) / (x - y))); solve_R.
    ...
    -- rewrite H1. simpl. field_simplify. apply limit_to_a_equiv with (f1 := λ _, 0); auto_limit.
    -- apply (limit_mult (λ x, y) _ _ y) in IH; try auto_limit.
