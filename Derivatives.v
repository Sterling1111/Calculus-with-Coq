...
Definition differentiable_at (f:R -> R) (a:R) :=
  exists L, ⟦ lim a ⟧ (fun h => (f (a + h) - f a) / h) = L.
...
Proof.
  intros f a [L H1]. apply lemma_9_1. assert (⟦ lim 0 ⟧ (fun h => (f (a + h) - f a) / h * h) = 0) as H2.
  { replace 0 with (L * 0) at 2 by lra. apply limit_mult. 2 : { apply limit_id. } 
    intros ε H2. specialize (H1 ε H2) as [δ [H1 H3]]. exists δ. split; auto. intros x H4.
Qed.